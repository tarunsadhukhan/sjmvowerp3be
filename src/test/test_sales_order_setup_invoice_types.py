"""
Tests for invoice_types in GET /api/salesOrder/get_sales_order_setup_1.
...
class TestSalesOrderSetupInvoiceTypes:
    ...
    def test_setup_returns_invoice_types(self):
        ...
        # 5. transporters, 6. co_config, 7. item_groups, 8. invoice_types
        branches = [_mock_row({"branch_id": 1, "branch_name": "Main"})]
        customers = [_mock_row({"party_id": 1, "party_name": "Cust A"})]
        cust_branches = []
        brokers = []
        transporters = []
        co_config = _mock_row({"quotation_required": 0})
